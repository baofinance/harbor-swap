...
contract BalancerSwapperForkTest is ForkTestBase, Swapper {
    ...
    function test_fork_swap_wstEthToWeth_executes() public {
        ...
        // The floor is a total in WETH, so the tolerance applies to the Vault's quote directly.
        uint256 minOut = (expected * 99) / 100;
        uint256 amountOut = ISwapExecutor(balancerSwapperProxy).swap(WSTETH, WETH, amountIn, minOut);
